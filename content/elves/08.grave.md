...
survivre à la mort, aussi bien pendant le runtime que lors d'un redémarrage du
monde.

> Survivre à la mort ? Hé oui, contrairement aux Goblins, les Elfes sont
> immortels.
...
Comme vous pouvez le constater, une ligne à changer et un propriété statique a
été ajoutée.
...
> Map et Set n'ont pas de représentations particulières sous la forme JSON. Il
> est nécessaire de travailler avec les types natifs comme `object` et `array`.
...
Je vois vos yeux. Tout va bien je vais tout vous expliquer. Dans le monde
Goblins il existe depuis bien longtemps un middleware pour Redux, qui se nomme
`Ripley`. Ce nom est un hommage à Ellen Ripley du film Alien. C'est surtout un
jeu de mot entre _replay_ et _ripley_. Le fait de rejouer, vient du concept des
actions stores.
...
de Goblin car ce n'est pas un nouveau mécanisme) pour autant que le Goblin a été
configuré explicitement pour que certaines actions soient sérialisées dans un
stockage Cryo.
...
d'action (au lieu que ce soit des événements) et en les rejouant les un après
...
> que ce systpme de transaction permet de gérer (par exemple) un formulaire avec
...
état est généré (un `persist`) qui ets alors une fusion de l'état d'origine sur
...
celui du client. Ce nouveau `persist` ets alors envoyé au client, il deviendra
