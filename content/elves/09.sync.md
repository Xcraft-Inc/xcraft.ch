...
Prenons Yeti, ce logiciel a de nombreux acteurs comme par exemple les demandes.
Une demande est un acteur de type Elf, qui a un état sérialisable (persistant).
Dans notre jargon interne on parle d'Archetype. Ici il faut simplement se dire
que c'est un acteur qui contient des informations sauvegardées sur l'ordinateur
local. Quand le programme est arrêté, les données de l'acteur ne sont pas
perdues. C’est très important car il existe aussi les acteurs de type Spirit qui
peuvent exister uniquement quand le programme est en cours de fonctionnement
(volatile).
...
vous présenter les mécaniques qui ont été mises en place avec Yeti.
...
Ceux-ci sont constitués de deux parties bien visibles : il y a les quêtes et les
reducers.
...
L'étape une est dites pure car les reducers sont des fonctions pures. Ce qui
...
sérialisables que l'on souhaite donner au reducer. Dans l'exemple du 'close', il
n'y a pas de payload particulier à transmettre car le type de l'action suffit.
...
Les actions qui ne sont pas de type "persist" permettent de modifier l'état de
...
Tous les "persist" sont alors retournés au client qui va les insérer dans sa
base de données.
